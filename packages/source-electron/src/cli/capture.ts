...
const captureHelpText = `
  ...
  Usage
    $ vishot-source-electron <scenario.ts> --app-entrypoint <electron-main> --output-dir <dir>

  Options
    --app-entrypoint Electron main entrypoint passed to Playwright Electron
  ...
  Examples
    $ vishot-source-electron ./scenarios/settings.ts --app-entrypoint ./dist/main.js --output-dir ./artifacts/raw
    $ vishot-source-electron ./scenarios/settings.ts --app-entrypoint ./dist/main.js --cwd ./apps/desktop -o ./artifacts/raw
`

const captureUsageMessage = 'Usage: vishot-source-electron <scenario.ts> --app-entrypoint <electron-main> --output-dir <dir>'
...
export function parseCaptureCliArguments(argv: string[]): CaptureCliArguments {
  ...
  if (cli.input.length !== 1
    || typeof cli.flags.appEntrypoint !== 'string'
    || cli.flags.appEntrypoint.length === 0
  ...
  return {
    ...
    appEntrypoint: cli.flags.appEntrypoint,
    cwd: cli.flags.cwd,
...
async function main(): Promise<void> {
  const { appEntrypoint, cwd, scenarioPath, outputDir, format, avif } = parseCaptureCliArguments(process.argv.slice(2))
  const resolvedCwd = cwd ? path.resolve(process.cwd(), cwd) : process.cwd()
  const resolvedAppEntrypoint = path.resolve(resolvedCwd, appEntrypoint)
  const resolvedOutputDir = path.resolve(process.cwd(), outputDir)

  await mkdir(resolvedOutputDir, { recursive: true })

  const loadedScenario = await loadScenarioModule(scenarioPath)
